 ...
 *
 * Changelog: 
 * v1.3 - changed gfx format to not be y-flipped by default
 */
...
void DoZlib(char *fileName)
{
	CSlrFile *file = new CSlrFileFromSystem(fileName);

	int inBufferNumBytes = file->GetFileSize();
	u8 *inBuffer = new u8[inBufferNumBytes];
	file->Read(inBuffer, inBufferNumBytes);
	file->Close();
	
	uLong outBufferSize = compressBound(inBufferNumBytes);
	byte *outBuffer = new byte[outBufferSize];

	int result = compress2(outBuffer, &outBufferSize, inBuffer, inBufferNumBytes, 9);
	...
	LOGD("..original size=%d compressed=%d", inBufferNumBytes, outSize);

	sprintf(buf, "%s.zlib", fileName);

	CByteBuffer *byteBuffer = new CByteBuffer();
	byteBuffer->PutU8('Z');
	byteBuffer->PutU8('L');
	byteBuffer->PutU32(inBufferNumBytes);
	byteBuffer->PutU32(outSize);
	byteBuffer->putBytes(outBuffer, outSize);
	
	byteBuffer->storeToFileNoHeader(buf);
	delete byteBuffer;
}
...
int main( int argc, char **argv )
{
	...
	LOGM("MTEngineSDL DeployMaker v" VERSION ", (C)2012 Marcin Skoczylas");
