	...
	//for ( ; itWidth != destScreenWidths.end(); itWidth++)
	{
		...
		for (u32 y = 0; y < loadImgHeight; y++)
		{
			for (u32 x = 0; x < loadImgWidth; x++)
			{
				...
				if (a > 0)
				{
					loadImageData->SetPixelResultRGBA(x, y, r, g, b, a);
					prevR = r;
					prevG = g;
					prevB = b;
				}
				else
				{
					...
					loadImageData->SetPixelResultRGBA(x, y, prevR, prevG, prevB, 0);
				}
			}
		}
...
void GFX_WriteImage(char *saveFileName, CSlrImageDummy *imageIn, u16 screenWidth, u16 destScreenWidth, byte compressionType, bool isSheet)
{
	...
	//for ( ; itWidth != destScreenWidths.end(); itWidth++)
	{
		...
		byteBuffer->putUnsignedShort(screenWidth);
		byteBuffer->putUnsignedShort(imageIn->origImageWidth);
		byteBuffer->putUnsignedShort(imageIn->origImageHeight);
		...
		if (destScreenWidth == screenWidth)
		{
			imageOut = imageIn->imageData;
		}
		else
		{
			scale = (float)destScreenWidth / (float)screenWidth;
			
			imageOut = IMG_Scale(imageIn->imageData, scale, scale, isSheet);
		}
		...
		u16 loadImgWidth = (u16)((float)imageIn->loadImgWidth * scale);
		u16 loadImgHeight = (u16)((float)imageIn->loadImgHeight * scale);
		byteBuffer->putUnsignedShort(loadImgWidth);
		byteBuffer->putUnsignedShort(loadImgHeight);
		byteBuffer->putUnsignedShort(rasterWidth);
		byteBuffer->putUnsignedShort(rasterHeight);
		
		CImageData *loadImageData = new CImageData(rasterWidth, rasterHeight, IMG_TYPE_RGBA);
		
		loadImageData->setRGBAResultData(imageOut->getRGBAResultData());
		...
		loadImageData->setRGBAResultData(NULL);
		
		delete loadImageData;
		
		if (imageOut != imageIn->imageData)
			delete imageOut;
		imgNum++;
