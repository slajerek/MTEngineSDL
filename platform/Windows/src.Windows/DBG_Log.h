...
// do {} while(0), NOT a bare ';': a macro that expands to just ';' eats
// only the first statement slot of a brace-less "if (x) LOGM(...); else
// ...;" -- the call site's own trailing ';' becomes a SECOND, sibling
