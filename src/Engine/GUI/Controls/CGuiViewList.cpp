...
CGuiViewList::CGuiViewList(float posX, float posY, float posZ, float sizeX, float sizeY,
...
bool CGuiViewList::DoMove(float x, float y, float distX, float distY, float diffX, float diffY)
{
	if (!visible)
		return false;
	...
	//////// DoMove
	float drawY = startDrawY + posY; // + startElementsY; //27;
	float drawX = startDrawX; //3;
...
bool CGuiViewList::FinishMove(float x, float y, float distX, float distY, float accelerationX, float accelerationY)
{
	if (!visible)
		return false;
...
bool CGuiViewList::DoZoomBy(float x, float y, float zoomValue, float difference)
{
	if (!visible)
		return false;
...
void CGuiViewList::MoveView(float diffX, float diffY)
{
	if (!visible)
		return;
...
//		 firstShowElement, this->listElements[firstShowElement]->sizeY, startDrawY);
	...
	float newStartDrawY = startDrawY + diffY;
...
bool CGuiViewList::DoTap(float x, float y)
{
	if (numElements < 1)
		return false;
	...
	float drawY = startDrawY + posY;  //+ startElementsY
	if (y < drawY)
		return false;
	...
	for (elemNum = firstShowElement; elemNum < numElements; elemNum++)
	{
		...
		float sy = (this->listElements[elemNum]->sizeY + this->listElements[elemNum]->gapY);
...
bool CGuiViewList::DoFinishTap(float x, float y)
{
	if (!visible)
		return false;
...
bool CGuiViewList::DoDoubleTap(float x, float y)
{
	if (!visible)
		return false;
