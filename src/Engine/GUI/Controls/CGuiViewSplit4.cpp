...
CGuiViewSplit4::CGuiViewSplit4(float posX, float posY, float posZ, float sizeX, float sizeY)
...
void CGuiViewSplit4::SetView(u8 viewNum, CGuiView *view)
{
...
void CGuiViewSplit4::Render()
{
	...
	for (u8 i = 0; i < 4; i++)
	{
		//LOGD("i=%d", i);
		if (i == 0)
			VID_SetClipping(0, 0, SCREEN_WIDTH/2, SCREEN_HEIGHT/2);
		else if (i == 1)
			VID_SetClipping(SCREEN_WIDTH/2, 0, SCREEN_WIDTH/2, SCREEN_HEIGHT/2);
		else if (i == 2)
			VID_SetClipping(0, SCREEN_HEIGHT/2, SCREEN_WIDTH/2, SCREEN_HEIGHT/2);
		else if (i == 3)
			VID_SetClipping(SCREEN_WIDTH/2, SCREEN_HEIGHT/2, SCREEN_WIDTH/2, SCREEN_HEIGHT/2);
...
void CGuiViewSplit4::Render(float posX, float posY)
{
	CGuiView::Render(posX, posY);
}

//@returns is consumed
bool CGuiViewSplit4::DoTap(float x, float y)
{
...
bool CGuiViewSplit4::DoFinishTap(float x, float y)
{
...
//@returns is consumed
bool CGuiViewSplit4::DoDoubleTap(float x, float y)
{
...
bool CGuiViewSplit4::DoFinishDoubleTap(float x, float y)
{
	LOGG("CGuiViewSplit4::DoFinishTap: %f %f", x, y);
...
bool CGuiViewSplit4::DoMove(float x, float y, float distX, float distY, float diffX, float diffY)
{
	float tx, ty;
	int screenNum;
	this->ConvertTap(x, y, &screenNum, &tx, &ty);
...
bool CGuiViewSplit4::FinishMove(float x, float y, float distX, float distY, float accelerationX, float accelerationY)
{
	float tx, ty;
	int screenNum;
	this->ConvertTap(x, y, &screenNum, &tx, &ty);
...
bool CGuiViewSplit4::DoZoomBy(float x, float y, float zoomValue, float difference)
{
	float tx, ty;
	int screenNum;
	this->ConvertTap(x, y, &screenNum, &tx, &ty);
