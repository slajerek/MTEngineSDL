...
int gPlatformType = PLATFORM_TYPE_TABLET;
...
CGuiTheme::CGuiTheme(const char *themeName)
{
	this->Init(themeName);
}

void CGuiTheme::Init(const char *themeName)
{
	imgBackground = NULL;
...
void CGuiTheme::LoadTheme(const char *themeName)
{
...
void CGuiTheme::LoadImageTheme(CSlrImage **image, const char *imageConfigName, const char *imageDefaultPath, bool linearScale)
{
...
void CGuiTheme::LoadFontTheme(CSlrImage **image, CSlrFont **font, const char *fontConfigName, const char *fontDefaultPath, bool linearScale)
{
	if ((*image) != NULL)
	{
		RES_ReleaseImage((*image), RESOURCE_LEVEL_THEME);
