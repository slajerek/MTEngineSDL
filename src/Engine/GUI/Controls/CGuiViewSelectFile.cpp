...
CGuiViewSelectFile::CGuiViewSelectFile(float posX, float posY, float posZ, float sizeX, float sizeY, bool cancelButton, CGuiViewSelectFileCallback *callback)
: CGuiView(posX, posY, posZ, sizeX, sizeY)
{
	...
	this->startDirectoryPath = new CSlrString("/");
...
void CGuiViewSelectFile::Init(std::list<CSlrString *> *extensions)
{
...
void CGuiViewSelectFile::Init(CSlrString *directoryPath, std::list<CSlrString *> *extensions)
{
	LOGD("CGuiViewSelectFile::Init");
	...
	else
	{
		...
		buf[1] = 0x00;
		this->currentDirectoryPath = new CSlrString(buf);
	}
...
void CGuiViewSelectFile::InitWithStartPath(CSlrString *directoryPath, std::list<CSlrString *> *extensions)
{
	...
	{
		delete this->startDirectoryPath;
	}
	
	if (this->currentDirectoryPath != NULL)
	{
		delete this->currentDirectoryPath;
	}
	
	this->startDirectoryPath = new CSlrString(directoryPath);		//strdup
	this->currentDirectoryPath = new CSlrString(directoryPath);	//strdup
...
//#endif
	...
	else
	{
		if (listBox->selectedElement == 0)
		{
			...
			CSlrString *newPath = new CSlrString(buf);
...
bool CGuiViewSelectFile::KeyDown(u32 keyCode, bool isShift, bool isAlt, bool isControl, bool isSuper)
{
...
void CGuiViewSelectFile::Render(float posX, float posY, float posZ, float sizeX, float sizeY)
{
	this->LockRenderMutex();
...
void CGuiViewSelectFileCallback::FileSelected(CSlrString *filePath)
{
	return;
}
