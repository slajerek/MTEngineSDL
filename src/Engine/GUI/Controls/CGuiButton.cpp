...
CGuiButton::CGuiButton(CSlrImage *image, float posX, float posY, float posZ, float sizeX, float sizeY, u8 alignment, CGuiButtonCallback *callback)
...
void CGuiButton::RecalcTextPosition()
{
	...
	else
	{
		...
		{
			float width, height;
			this->font->GetTextSize(this->textUTF, this->fontScale, &width, &height);
			
			if (centerText)
			{
				textDrawPosX = (sizeX / 2.0f);	// hmm... I guess it is elsewhere?: // - (width / 2.0f);
			}
			else
			{
				textDrawPosX = 0.0f;
		...
		else
		{
			...
			{
				float width, height;
				guiMain->fntEngineDefault->GetTextSize(this->textUTF, this->fontScale, &width, &height);
...
void CGuiButton::RenderText(float posX, float posY)
{
	if (this->text2 != NULL)
	{
		if (this->font == NULL)
...
void CGuiButton::RenderUTFButton(float posX, float posY)
{
...
void CGuiButton::Render(float posX, float posY)
{
	if (this->visible)
	{
		if (this->textUTF != NULL)
...
// @returns is consumed
bool CGuiButton::DoTap(float posX, float posY)
{
	if (!this->visible)
		return false;
...
// @returns is consumed
bool CGuiButton::DoDoubleTap(float posX, float posY)
{
	if (!this->visible)
		return false;
...
bool CGuiButton::DoFinishDoubleTap(float posX, float posY)
{
	if (!this->visible)
		return false;
...
bool CGuiButton::Clicked(float posX, float posY)
{
	LOGG("CGuiButton::Clicked: %f %f", posX, posY);
	return false;
}

bool CGuiButton::Pressed(float posX, float posY)
{
	LOGG("CGuiButton::Pressed: %f %f", posX, posY);
	return false;
}

bool CGuiButton::DoMove(float x, float y, float distX, float distY, float diffX, float diffY)
{
	if (!this->visible)
		return false;
...
bool CGuiButton::FinishMove(float x, float y, float distX, float distY, float accelerationX, float accelerationY)
{
	if (!this->visible)
		return false;
