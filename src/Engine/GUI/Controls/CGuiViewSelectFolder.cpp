...
CGuiViewSelectFolder::CGuiViewSelectFolder(float posX, float posY, float posZ, float sizeX, float sizeY, bool cancelButton, CGuiViewSelectFolderCallback *callback)
...
#if !defined(IOS)
	this->startDirectoryPath = new CSlrString("/");		//strdup
	this->currentDirectoryPath = new CSlrString("/");	//strdup
#else
...
void CGuiViewSelectFolder::Init(CSlrString *directoryPath)
{
	LOGD("CGuiViewSelectFolder::Init");
...
//	this->startDirectoryPath = UTFALLOC(gPathToDocuments);		//strdup
	this->startDirectoryPath = new CSlrString("/");		//strdup
	this->currentDirectoryPath = new CSlrString(directoryPath);	//strdup
...
void CGuiViewSelectFolder::Init(CSlrString *startPath, CSlrString *currentPath)
{
	LOGD("CGuiViewSelectFolder::Init");
	
	this->LockRenderMutex();
	this->startDirectoryPath = new CSlrString(startPath);		//strdup
	this->currentDirectoryPath = new CSlrString(currentPath);	//strdup
...
void CGuiViewSelectFolder::UpdatePath()
{
	...
	{
//		startDirectoryPath = new CSlrString(gPathToDocuments);
		startDirectoryPath = new CSlrString("/");
	}
	
	if (currentDirectoryPath == NULL)
	{
		currentDirectoryPath = new CSlrString("/");
	}
#else
	...
	this->files = SYS_GetFilesInFolder(currentDirectoryPath, &(extensions));
...
void CGuiViewSelectFolder::ListElementSelected(CGuiList *listBox)
{
	...
	SYS_FatalExit("ListElementSelected");
	
	/*
	 CFileItem *file = NULL;
	
	if (currentDirectoryPath->Equals(startDirectoryPath))
	{
...
{
	SYS_FatalExit("UpdateDisplayDirectoryPath");
...
bool CGuiViewSelectFolder::KeyDown(u32 keyCode, bool isShift, bool isAlt, bool isControl, bool isSuper)
{
...
void CGuiViewSelectFolder::Render(float posX, float posY, float posZ, float sizeX, float sizeY)
{
	this->LockRenderMutex();
...
void CGuiViewSelectFolderCallback::FolderSelected(CSlrString *fullFolderPath, CSlrString *folderPath)
{
	return;
}
