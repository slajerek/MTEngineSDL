...
CGuiListTree::CGuiListTree(float posX, float posY, float posZ, float sizeX, float sizeY, float fontSize,
...
bool CGuiListTree::DoMove(float x, float y, float distX, float distY, float diffX, float diffY)
{
	if (!visible)
		return false;
...
bool CGuiListTree::FinishMove(float x, float y, float distX, float distY, float accelerationX, float accelerationY)
{
	if (!visible)
		return false;
...
bool CGuiListTree::DoZoomBy(float x, float y, float zoomValue, float difference)
{
	if (!visible)
		return false;
...
void CGuiListTree::MoveView(float diffX, float diffY)
{
	if (!visible)
		return;
...
bool CGuiListTree::DoTap(float x, float y)
{
	scrollModified = false;
	return CGuiElement::DoTap(x, y);
}

bool CGuiListTree::DoFinishTap(float x, float y)
{
	...
	float drawY = startDrawY + startElementsY + posY;
	if (y < drawY)
	{
	...
	if (!elem->elements.empty())
	{
		...
		float px = this->posX + startDrawX + elem->openerSpaces * this->fontSize;
...
bool CGuiListTree::DoDoubleTap(float x, float y)
{
	if (!visible)
		return false;
...
//	byte line = 0;
	...
	float drawY = startDrawY + startElementsY; //27;
