#include "CDevelopBaselineSeams.h"
...
// ---------------------------------------------------------------------------
// Seam 1 baseline: CBaselineCameraInputs
// ---------------------------------------------------------------------------

bool CBaselineCameraInputs::Resolve(const SRawDngTags &file,
                                    const SWhiteBalanceRequest &requested,
                                    SCameraResolution *out)
{
	*out = SCameraResolution();   // offset 0, identity calibration, unit AB

	// --- referenceNeutral (#4.0/#4.1; plan F2's formula, G = 1) -------------
	bool haveNeutral = false;
	float n[3] = { 1.f, 1.f, 1.f };

	if (requested.mode == SWhiteBalanceRequest::TempTint)
	{
		// Kelvin only: the +/-100 relative scale has no meaning without a
		// profile reference (#4.1), and without a matrix there is no
		// Kelvin->neutral conversion at all (rev 8 F2).
		if (requested.temperatureIsKelvin && file.hasMatrix)
		{
			SDevTempTint tt;
			tt.temperatureK = requested.temperature;
			tt.tint         = requested.tint;
			haveNeutral = PC_TempTintToCameraNeutral(tt, file.camToXYZ, n);
		}
		if (!haveNeutral)
			out->warnedPresetFallback = true;   // documented: also the
			                                    // no-matrix / non-Kelvin arm
	}
	else if (requested.mode == SWhiteBalanceRequest::Preset)
	{
		// A named preset with no Temp/Tint: the baseline cannot derive a
		// preset's Kelvin from a single matrix (#4.1) -- as-shot + warning.
		out->warnedPresetFallback = true;
	}

	if (!haveNeutral)
	{
		// As-shot (and every fallback): normalise_G(1 / relativeWB) -- the
		// multipliers LibRaw ACTUALLY applied, so stages 2 and 3b compose to
		// identity at As Shot BY CONSTRUCTION (plan F2; the flat-neutral-field
		// test falsifies an inversion here, the identity test cannot).
		// NOT dngAsShotNeutral: that describes the shot illuminant for RD-D's
		// NeutralToXY fixed point; the RENDER neutral must be the
		// undo-consistent one or As Shot stops being a no-op.
		const float gMul = (file.relativeWB[1] > 1e-9f) ? file.relativeWB[1] : 1.f;
		for (int c = 0; c < 3; c++)
		{
			const float m = (file.relativeWB[c] > 1e-9f) ? file.relativeWB[c] : gMul;
			n[c] = gMul / m;   // (1/m_c) / (1/m_G)
		}
		n[1] = 1.f;
	}
	memcpy(out->referenceNeutral, n, sizeof(n));

	// --- diagnostics: the resolved white as xy / CCT / tint -----------------
	if (file.hasMatrix)
	{
		float xyz[3];
		PC_Mat3Apply(file.camToXYZ, n, xyz);
		const float s = xyz[0] + xyz[1] + xyz[2];
		if (s > 1e-9f)
		{
			out->whiteXY[0] = xyz[0] / s;
			out->whiteXY[1] = xyz[1] / s;
			SDevTempTint tt;
			if (PC_XyToTempTint(out->whiteXY, &tt))
			{
...
// ---------------------------------------------------------------------------
// Seam 2 baseline: CBaselineMatrixStage
// ---------------------------------------------------------------------------

CBaselineMatrixStage::CBaselineMatrixStage(const float camToXYZ[3][3], bool hasMatrix)
	: hasMatrix_(hasMatrix)
{
	// Bradford folded AT CONSTRUCTION (#5 "Why XYZ(D50)"): LibRaw's matrix is
	// D65-normalised (RD-A #6.1), the seam contract is XYZ(D50). One 3x3.
	if (hasMatrix_)
		PC_Mat3Mul(PC_kBradfordD65ToD50, camToXYZ, folded_);
	else
		memset(folded_, 0, sizeof(folded_));   // caller refuses before use
}

void CBaselineMatrixStage::GetMatrix(float outM[3][3])
{
	memcpy(outM, folded_, sizeof(folded_));
}

// ---------------------------------------------------------------------------
// Seam 5 baseline: the published acr3 default tone curve (#4.8)
// ---------------------------------------------------------------------------
//
// PROVENANCE: dng_tone_curve_acr3_default::Evaluate's kTable, Adobe DNG SDK
// (public; fetched 2026-08-13 from the aizvorski/dng_sdk mirror of
// dng_render.cpp). 1025 entries, linear domain, endpoints exactly 0 and 1,
// monotone nondecreasing -- all pinned by tests so a transcription error
// cannot ship silently. Published DNG SDK DATA, engine-eligible by the #5.1
// dng_temperature boundary ruling (plan F4); dng_sdk itself is NOT vendored.

static const int kAcr3TableSize = 1025;
static const float kAcr3Table[kAcr3TableSize] = {
...
{
	if (outCount != nullptr)
		*outCount = kAcr3TableSize;
	return kAcr3Table;
}

float PC_Acr3DefaultCurveEvaluate(float x)
{
	// Inside [0,1]: the published Evaluate semantics verbatim (scale by
	// tableSize-1, lerp). Outside: slope extension -- the published code
	// clamps because dng_render's input is display-limited; ours is
	// scene-referred (#2.2), and clamping here would be the silent clipper
	// the float container was chosen to avoid. Endpoints and interior are
	// identical to the published curve.
	if (x <= 0.f)
	{
		const float startSlope = (kAcr3Table[1] - kAcr3Table[0]) * (float)(kAcr3TableSize - 1);
		return x * startSlope;
	}
	if (x >= 1.f)
	{
		const float endSlope = (kAcr3Table[kAcr3TableSize - 1] - kAcr3Table[kAcr3TableSize - 2])
		                     * (float)(kAcr3TableSize - 1);
		return 1.f + (x - 1.f) * endSlope;
	}
	const float y = x * (float)(kAcr3TableSize - 1);
	int index = (int)y;
	if (index > kAcr3TableSize - 2) index = kAcr3TableSize - 2;
	const float fract = y - (float)index;
	return kAcr3Table[index] * (1.f - fract) + kAcr3Table[index + 1] * fract;
}

std::function<float(float)> CBaselineToneCurve::GetLinearCurve()
{
	if (!enabled_)
		return std::function<float(float)>();   // stage 9 skipped (#4.8, #9.1)
	return [](float x) { return PC_Acr3DefaultCurveEvaluate(x); };
}
