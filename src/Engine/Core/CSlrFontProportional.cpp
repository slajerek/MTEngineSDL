...
CSlrFontProportional::CSlrFontProportional(bool fromResources, const char *fontPath)
{
...
CSlrFontProportional::CSlrFontProportional(bool fromResources, const char *fontPath, bool linearScale)
{
	this->fontType = FONT_TYPE_PROPORTIONAL;
	this->name = strdup(fontPath);
	this->texturePageImageData = NULL;
...
void CSlrFontProportional::StoreFontDataToByteBuffer(CByteBuffer *byteBuffer, u8 proportionalFontType)
{
...
void CSlrFontProportional::StoreFontAndTexture(CImageData *image, CByteBuffer *fontData)
{
	...
	image->StoreToByteBufferUncompressed(fontData);
}
...
void CSlrFontProportional::BlitChar(char chr, float posX, float posY, float posZ, float size, float alpha)
{
	...
	this->texturePage->RenderAlpha(posX + (f->xOffset*size), posY + (f->yOffset*size), posZ,
			f->width * size * scaleAdjust, f->height * size * scaleAdjust,
			texAdvX*f->x,
			texAdvY*f->y,
			texAdvX*(f->x + f->width),
...
void CSlrFontProportional::BlitChar(u16 chr, float posX, float posY, float posZ, float size)
{
...
void CSlrFontProportional::BlitChar(u16 chr, float posX, float posY, float posZ, float size, float alpha)
{
	CharDescriptor *f = GetCharDescriptorInt(chr);
	if (f == NULL)
		return;
...
void CSlrFontProportional::BlitCharColor(u16 chr, float posX, float posY, float posZ, float size, float r, float g, float b, float alpha)
{
	CharDescriptor *f = GetCharDescriptorInt(chr);
	if (f == NULL)
		return;
...
void CSlrFontProportional::BlitText(const char *text, float posX, float posY, float posZ, float scale)
{
	...
	for (u16 i = 0; i < l; i++)
	{
		...
		this->texturePage->Render(curX + (f->xOffset * scale * scaleAdjust), curY + (f->yOffset * scale * scaleAdjust), posZ,
...
void CSlrFontProportional::BlitText(const char *text, float posX, float posY, float posZ, float scaleX, float scaleY, float alpha)
{
	float advX = (float) 1.0f/width;
	float advY = (float) 1.0f/height;
...
void CSlrFontProportional::BlitText(const char *text, float posX, float posY, float posZ, float scale, float alpha)
{
	float advX = (float) 1.0f/width;
	float advY = (float) 1.0f/height;
...
void CSlrFontProportional::BlitTextColor(const char *text, float posX, float posY, float posZ, float scale, float colorR, float colorG, float colorB, float alpha)
{
	float advX = (float) 1.0f/width;
	float advY = (float) 1.0f/height;
...
void CSlrFontProportional::BlitTextColor(CSlrString *text, float posX, float posY, float posZ, float scale, float colorR, float colorG, float colorB, float alpha)
{
	this->BlitTextColor(text, posX, posY, posZ, scale, colorR, colorG, colorB, alpha, FONT_ALIGN_LEFT);
}

void CSlrFontProportional::BlitTextColor(CSlrString *text, float posX, float posY, float posZ, float scale, float colorR, float colorG, float colorB, float alpha, int align)
{
	float advX = (float) 1.0f/width;
	float advY = (float) 1.0f/height;
...
void CSlrFontProportional::BlitTextColor(CSlrString *text, float posX, float posY, float posZ, float scale, float advance, float colorR, float colorG, float colorB, float alpha)
{
	float advX = (float) 1.0f/width;
	float advY = (float) 1.0f/height;
...
void CSlrFontProportional::BlitText(const char *text, float x, float y, float z,
...
float CSlrFontProportional::GetTextWidth(const char *text, float scale)
{
...
void CSlrFontProportional::GetTextSize(const char *text, float scale, u16 *width, u16 *height)
{
	*width = this->GetTextWidth(text, scale);
	*height = (u16)((float)this->lineHeight * scale * scaleAdjust);
}

void CSlrFontProportional::GetTextSize(const char *text, float scale, float *width, float *height)
{
	*width = this->GetTextWidth(text, scale);
	*height = ((float)this->fontHeight * scale * scaleAdjust);
}
...
void CSlrFontProportional::ResourceSetPriority(u8 newPriority)
{
