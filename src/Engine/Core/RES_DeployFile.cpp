...
void RES_DeployFileLoad(CByteBuffer *deployBuffer, u16 destScreenSize)
{
	...
	for (u32 i = 0; i < numFiles; i++)
	{
		...
		if (type == DEPLOY_FILE_TYPE_GFX)
		{
			...
			bool forceOriginal = deployBuffer->getBool();
...
void RES_AddEmbeddedDataToDeploy(const char *fileName, u16 fileType, uint8 *embeddedData, int embeddedDataLength)
{
	CDeployFileDetails *deployFileDetails = NULL;
	...
	LOGR("RES_AddFileToDeploy: resource added %lld-%2.2x", hashCode, type);
}

CSlrFile *RES_GetFileFromDeploy(const char *fileName, u8 fileType)
{
...
const char *RES_GetFileTypeExtension(u8 fileType)
{
	switch(fileType)
	{
...
CDeployFileDetails::CDeployFileDetails(u64 hashCode, uint8 fileType, char *fileExtStr, bool forceOriginal, u8 *embeddedData, int embeddedDataLength)
{
#if !defined(FINAL_RELEASE)
	char hashcodeStr[512];
	sprintfHexCode64(hashcodeStr, hashCode);
