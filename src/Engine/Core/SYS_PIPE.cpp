...
#define FIFO_WRITE_PIPE_PATH "/tmp/%s-%lu-out.pipe"
#define FIFO_READ_PIPE_PATH "/tmp/%s-%lu-in.pipe"
...
HANDLE hPipeRead;
DWORD dwRead;
#define FIFO_WRITE_PIPE_PATH "\\\\.\\pipe\\%s-%d-out"
#define FIFO_READ_PIPE_PATH "\\\\.\\pipe\\%s-%d-in"
...
void PIPE_Init(const char *pipeName)
{
	...
	sprintf(pipeWritePath, FIFO_WRITE_PIPE_PATH, pipeName, pid);
	sprintf(pipeReadPath, FIFO_READ_PIPE_PATH, pipeName, pid);
	...
	if (SYS_FileExists(pipeWritePath))
	{
		sprintf(buf, "rm -rf %s 2> /dev/null", pipeWritePath);
		system(buf);
	}
	...
	if (SYS_FileExists(pipeReadPath))
	{
		sprintf(buf, "rm -rf %s 2> /dev/null", pipeReadPath);
		system(buf);
	}
