...
volatile u8 gResourceManagerState = RESOURCE_MANAGER_STATE_IDLE;

volatile u8 gResourceDefaultLevel = RESOURCE_PRIORITY_NORMAL;
...
CImageLoadData::CImageLoadData(const char *imageName, bool linearScaling)
{
...
CSlrImage *RES_GetImageAsync(const char *imageNameConst, bool linearScaling, int resourceLevel, bool fromResources)
{
	LOGR("RES_GetImageAsync: '%s' level=%d fromResources=%s", imageNameConst, resourceLevel, (fromResources ? "true" : "false"));
	
	RES_LockMutex("RES_GetImageAsync");
	
	char *imageName = SYS_GetCharBuf();
	strcpy(imageName, imageNameConst);
...
void RES_RegisterImageFromAtlas(CSlrImage *imageAtlas, const char *name, int startPosX, int startPosY, int endPosX, int endPosY, float scale)
{
	...
	CSlrImage *imgFromAtlas = new CSlrImage(imageAtlas, (float)startPosX, (float)startPosY, (float)endPosX, (float)endPosY, scale, (char*)name);
...
CSlrFile *RES_GetFile(const char *fileName, u8 fileType)
{
	...
	if (file->Exists())
	{
		LOGR("RES_GetFile: opened '%s'", fileName);
		return file;
	}
...
CSlrFileZlib *RES_GetFileZlib(const char *fileName, u8 fileType)
{
	...
	u8 m1 = byteBuffer->GetU8();
	u8 m2 = byteBuffer->GetU8();
	if (m1 != 'Z' || m2 != 'L')
	{
		LOGError("RES_GetZlibFile: ZL magic not found");
		delete byteBuffer;
		return NULL;
	}
	
	u32 uncompressedBufferSize = byteBuffer->GetU32();
	u32 compressedBufferSize = byteBuffer->GetU32();
	
	CSlrFileFromMemory *fileMem = new CSlrFileFromMemory(byteBuffer);
	CSlrFileZlib *fileZlib = new CSlrFileZlib(fileMem, uncompressedBufferSize, true);
	
	byteBuffer->data = NULL;
	delete byteBuffer;
	
	LOGR("RES_GetZlibFile: %s %d | compressed=%d uncompressed=%d", fileName, fileType, compressedBufferSize, uncompressedBufferSize);
...
u32 RES_PrepareMemory(u32 memoryNeeded, bool async)
{
	...
	if (totalMem > gMaxTotalSystemMemory)
	{
					...
					it != resourcesSortedByActivation.end(); it++)
		{
			...
			// TODO: RESOURCE MANAGER guiMain->viewResourceManager->RefreshDataTable();
...
void RES_ResourcesPreparingFinishedForLoadingScreen()
{
		 ...
		 it != resourcesToLoad.end(); it++)
	{
		...
		// TODO: RESOURCE MANAGER guiMain->viewLoadingScreen->SetLoadingText(res->ResourceGetPath());
...
void RES_LoadResourcesAsync()
{
	...
	// TODO: RESOURCE MANAGER guiMain->viewLoadingScreen->SetLoadingText("calculating");
	...
	// TODO: RESOURCE MANAGER guiMain->viewLoadingScreen->SetLoadingText("deallocating");
	...
	for (  ; numPass < 5; numPass++)
	{
		...
		u64 memFreed = RES_PrepareMemory(memToDeactivate, true);
	...
	// TODO: RESOURCE MANAGER guiMain->viewLoadingScreen->SetLoadingText("activating");
	...
	// TODO: RESOURCE MANAGER guiMain->viewLoadingScreen->SetLoadingText("binding");
	...
	// TODO: RESOURCE MANAGER guiMain->viewLoadingScreen->SetLoadingText("idle");
