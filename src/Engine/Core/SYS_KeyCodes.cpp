...
CSlrString *SYS_KeyCodeToString(u32 keyCode, bool isShift, bool isAlt, bool isControl, bool isSuper)
{
	...
	if (isSuper || (keyCode & MTKEY_SPECIAL_SUPER) == MTKEY_SPECIAL_SUPER)
	{
#if defined(MACOS)
		strOut->Concatenate("Ctrl+");
#else
...
{
	LOGI("SYS_KeyCodeConvertSpecial: %x %d %d %d %d", keyCode, isShift, isAlt, isControl, isSuper);
...
{
	if (keyCode == MTKEY_SPECIAL_SHIFT
		|| keyCode == MTKEY_SPECIAL_CONTROL
		|| keyCode == MTKEY_SPECIAL_ALT
		|| keyCode == MTKEY_SPECIAL_SUPER
		|| keyCode == MTKEY_LSHIFT
