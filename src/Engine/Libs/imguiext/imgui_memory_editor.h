...
struct MemoryEditor
{
    ...
    void DrawPreviewData(size_t addr, const ImU8* mem_data, size_t mem_size, ImGuiDataType data_type, DataFormat data_format, char* out_buf, size_t out_buf_size) const
    {
        ...
        {
            uint8_t v = 0;
            EndianessCopy(&v, buf, size);
            if (data_format == DataFormat_Dec) { ImSnprintf(out_buf, out_buf_size, "%hhu", v); return; }
            if (data_format == DataFormat_Hex) { ImSnprintf(out_buf, out_buf_size, "0x%02x", v & 0XFF); return; }
            break;
        }
        case ImGuiDataType_S16:
        {
            int16_t v = 0;
            EndianessCopy(&v, buf, size);
            if (data_format == DataFormat_Dec) { ImSnprintf(out_buf, out_buf_size, "%hd", v); return; }
            if (data_format == DataFormat_Hex) { ImSnprintf(out_buf, out_buf_size, "0x%04x", v & 0xFFFF); return; }
            break;
        }
        case ImGuiDataType_U16:
        {
            uint16_t v = 0;
            EndianessCopy(&v, buf, size);
            if (data_format == DataFormat_Dec) { ImSnprintf(out_buf, out_buf_size, "%hu", v); return; }
            if (data_format == DataFormat_Hex) { ImSnprintf(out_buf, out_buf_size, "0x%04x", v & 0xFFFF); return; }
            break;
        }
        case ImGuiDataType_S32:
        {
            int32_t v = 0;
            EndianessCopy(&v, buf, size);
            if (data_format == DataFormat_Dec) { ImSnprintf(out_buf, out_buf_size, "%d", v); return; }
            if (data_format == DataFormat_Hex) { ImSnprintf(out_buf, out_buf_size, "0x%08x", v); return; }
            break;
        }
        case ImGuiDataType_U32:
        {
            uint32_t v = 0;
            EndianessCopy(&v, buf, size);
            if (data_format == DataFormat_Dec) { ImSnprintf(out_buf, out_buf_size, "%u", v); return; }
            if (data_format == DataFormat_Hex) { ImSnprintf(out_buf, out_buf_size, "0x%08x", v); return; }
            break;
        }
        case ImGuiDataType_S64:
        {
            int64_t v = 0;
            EndianessCopy(&v, buf, size);
            if (data_format == DataFormat_Dec) { ImSnprintf(out_buf, out_buf_size, "%lld", (long long)v); return; }
            if (data_format == DataFormat_Hex) { ImSnprintf(out_buf, out_buf_size, "0x%016llx", (long long)v); return; }
            break;
        }
        case ImGuiDataType_U64:
        {
            uint64_t v = 0;
            EndianessCopy(&v, buf, size);
            if (data_format == DataFormat_Dec) { ImSnprintf(out_buf, out_buf_size, "%llu", (long long)v); return; }
            if (data_format == DataFormat_Hex) { ImSnprintf(out_buf, out_buf_size, "0x%016llx", (long long)v); return; }
            break;
        }
        case ImGuiDataType_Float:
        {
            float v = 0.0f;
            EndianessCopy(&v, buf, size);
            if (data_format == DataFormat_Dec) { ImSnprintf(out_buf, out_buf_size, "%f", v); return; }
            if (data_format == DataFormat_Hex) { ImSnprintf(out_buf, out_buf_size, "%a", v); return; }
            break;
        }
