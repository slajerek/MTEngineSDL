...
// [MTENGINE-PATCH: sdl3-viewport-to-window] not declared in imgui_impl_sdl3.h -- callers declare the prototype themselves
// Exported (non-static) wrapper over upstream's own ImGui_ImplSDL3_GetSDLWindowFromViewport,
// so engine code can reach the SDL window behind a viewport.
//
// The SDL2 version of this patch reached into ViewportData->Window itself. SDL3's
// backend already has a helper that does the same job via PlatformHandle/WindowID,
// so this patch shrank to a one-line forward -- less of our code to keep correct
// across ImGui upgrades, and it exercises upstream's own lookup rather than a
// parallel one that could drift from it.
SDL_Window *ImGui_ImplSDL3_ImGuiViewportToSDLWindow(ImGuiViewport *imGuiViewport)
{
	return ImGui_ImplSDL3_GetSDLWindowFromViewport(imGuiViewport);
}
// [/MTENGINE-PATCH: sdl3-viewport-to-window]
