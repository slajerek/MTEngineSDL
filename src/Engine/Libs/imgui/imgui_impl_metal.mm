...
bool ImGui_ImplMetal_CreateDeviceObjects(id<MTLDevice> device)
{
    ...
    // [MTENGINE-PATCH: metal-ktx2-mipmaps] comment-only: the mipFilter line BELOW is
    // load-bearing for KTX2 mipmaps -- the patch is this warning, not that line, which is
    // unmodified upstream.
    //
    // UPGRADE NOTE (1.92.6 -> 1.93.0): upstream REPLACED the hard-coded
    // `constexpr sampler ... mip_filter::linear` in the shader source with real
    // MTLSamplerState objects, and now builds TWO -- linear and nearest, selectable
    // per draw via ImGui_ImplMetal_DrawCallback_SetSampler{Linear,Nearest}. The
    // trilinear behaviour the engine needs for GPU-compressed mip chains
    // (KTX2/UASTC -> BC7/ASTC) survives as samplerStateLinear, which is the default
    // bound per frame. DO NOT drop MTLSamplerMipFilterLinear here, and do not switch
    // a mipped KTX2 texture to the NEAREST sampler -- that one is MipFilterNearest and
    // would point-sample the mip chain.
    samplerDescriptor.mipFilter = MTLSamplerMipFilterLinear;
    // [/MTENGINE-PATCH: metal-ktx2-mipmaps]
