...
#  define ZLIB_CONST
#endif
/* MTEngineSDL local patch, carried forward from the 1.5.2 vendoring.
 * A relative path to the VENDORED zlib, not a bare "zlib.h": the store
 * policy is that we build our own third-party libraries, and a bare
 * include would resolve against whatever zlib the include path happens
 * to reach first -- a system or Homebrew copy on some machines. This is
 * the one patch that must survive every libpng upgrade.
 */
#include "./../zlib/zlib.h"
#ifdef const
