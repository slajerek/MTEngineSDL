...
// removed by Slajerek as this does not work: | ImGuiWindowFlags_NoBringToFrontOnFocus
...
// this is defined in CGuiMain now
...
public:
	...
	NOTIFY_INLINE auto get_elapsed_time() { return SYS_GetTickCount() - this->creation_time; }
	...
	{
		const ImGuiToastPhase phase = get_phase();
		const auto elapsed = get_elapsed_time();
...
public:
	...
	ImGuiToast(ImGuiToastType type, int dismiss_time = NOTIFY_DEFAULT_DISMISS)
	{
		...
		this->creation_time = SYS_GetTickCount();
...
//		NOTIFY_FORMAT(this->set_content, format);
	...
	ImGuiToast(ImGuiToastType type, const char *title, int dismiss_time, const char* format, ...) : ImGuiToast(type, dismiss_time) {
...
//		notifications.push_back(toast);
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		{
			vp_size.x = VID_GetScreenWidth();
			vp_size.y = VID_GetScreenHeight();
		}
		else
		{
			vp_size = GetMainViewport()->Size;
		}
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			char window_name[50];
			ImFormatString(window_name, 50, "##TOAST%d", i);
