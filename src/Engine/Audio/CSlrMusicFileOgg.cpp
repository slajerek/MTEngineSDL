...
size_t _mt_OggReadFunct(void *ptr, size_t size, size_t nmemb, void *datasource)
{
	...
	return (size_t)file->Read((u8*)ptr, size*nmemb);
}
...
bool CSlrMusicFileOgg::Init(const char *fileName, bool seekable, bool fromResources)
{
...
//	return;
	...
	while (samplePos < numSamples)
	{
		if (oggAudioBufferPos >= oggAudioBufferLen)
		{
			...
			{
				memset((u8 *) oggMixBuffer, 0x00, len);
				oggAudioBufferPos = SOUND_BUFFER_SIZE;
